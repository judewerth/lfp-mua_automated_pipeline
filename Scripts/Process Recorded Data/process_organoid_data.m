...
windowdur = 0.25; %s; half overlap (0.125 s)
frequency_bands = { ...
    [1, 4], ... %delta
    [4, 7], ... %theta
...
%frequencies = [1 : 1 : 19, 20 : 2 : 38, 40 : 4 : 76, 80 : 8 : 200];
...
%spike_thrs = [];
spike_thrs = [-35, 4i, 5i]; %uV; imaginary values stand for the number of sigmas to be exceeded (e.g., 5i: abs(signal) > 5 sigma)
    ...
    for k = 1 : K
        ...
        try
            ...
            if ~RHDformat
                ...
                data = read_Intan_RHD2000_file_JK(filenames(k).name, notch_filter_frequencies); %run without a notch
            end
        end
        ...
        [features, time, spikewaveforms, spikewaveformscell, rms, windowL, overlap] = derive_features(data, frequency_bands, spike_thrs, windowdur, rereference, savespikewaveforms);

        if strcmp(name(end), '*') %remove the wildcart
            name(end) = [];
        end
        fileout = sprintf('%s', filenames(k).name);
        fileout(end - 2 : end) = 'mat';
        fprintf('Saving %s\n', fileout);
        ...
        if savespikewaveforms
            if ~RHDformat
                ...
                save(fileout, 'Z', 'features', 'time', 'frequency_bands', 'spike_thrs', 'spikewaveformscell', 'fs', 'rms', 'windowL', 'overlap');
            end
        else
            if ~RHDformat
                save(fileout, 'Z', 'stds', 'features', 'time', 'frequency_bands', 'spike_thrs', 'fs', 'rms', 'windowL', 'overlap', 'stimtrials', 'stimtimes', 'estimtimes');
            else
                save(fileout, 'Z', 'features', 'time', 'frequency_bands', 'spike_thrs', 'fs', 'rms', 'windowL', 'overlap');
            end
        end

        %mark the file as processed
        file1 = sprintf('%s', filenames(k).name);
        file2 = sprintf('processed_%s', filenames(k).name);
        movefile(file1, file2, 'f');
    end
